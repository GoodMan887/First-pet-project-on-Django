...
class UserLoginForm(AuthenticationForm):
    ...
    password = forms.CharField(widget=PasswordInput(attrs={
        'class': 'from-control py-4', 'placeholder': "Введите пароль"}))
...
class UserRegistrationForm(UserCreationForm):
    first_name = forms.CharField(widget=TextInput(attrs={
        'class': 'from-control py-4', 'placeholder': "Введите имя"}))
    last_name = forms.CharField(widget=TextInput(attrs={
        'class': 'from-control py-4', 'placeholder': "Введите фамилию"}))
    username = forms.CharField(widget=TextInput(attrs={
        'class': 'from-control py-4', 'placeholder': "Введите имя пользователя"}))
    email = forms.CharField(widget=EmailInput(attrs={
        'class': 'from-control py-4', 'placeholder': "Введите адрес эл. почты"}))
    password1 = forms.CharField(widget=PasswordInput(attrs={
        'class': 'from-control py-4', 'placeholder': "Введите пароль"}))
    password2 = forms.CharField(widget=PasswordInput(attrs={
        'class': 'from-control py-4', 'placeholder': "Подтвердите пароль"}))
